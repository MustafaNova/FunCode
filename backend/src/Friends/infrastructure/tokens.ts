export const USER_LOOK_UP_PORT = 'USER_LOOK_UP_PORT' as const;
export const FRIEND_REQUEST_REPO_PORT = 'FRIEND_REQUEST_REPO_PORT' as const;
export const FRIENDSHIP_REPO_PORT = 'FRIENDSHIP_REPO_PORT' as const;
export const FRIEND_REQ_TX_PORT = 'FRIEND_REQ_TX_PORT' as const;

// UC-WIRING TOKENS
export const CREATE_FRIEND_REQ_PORT = 'CREATE_FRIEND_REQ_PORT' as const;
export const GET_INCOMING_FRIEND_REQ_PORT = 'GET_INCOMING_FRIEND_REQ_PORT' as const;
export const ACCEPT_FRIEND_REQ_PORT = 'ACCEPT_FRIEND_REQ_PORT' as const;
export const GET_FRIENDS_PORT = 'GET_FRIENDS_PORT' as const;
export const DECLINE_FRIEND_REQ_PORT = 'DECLINE_FRIEND_REQ_PORT' as const;
export const DELETE_FRIEND_PORT = 'DELETE_FRIEND_PORT' as const;
